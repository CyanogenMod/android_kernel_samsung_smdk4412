...
static int rmi_f1a_alloc_memory(struct rmi_function_container *fc)
{
	...
	f1a = devm_kzalloc(&fc->dev, sizeof(struct f1a_data), GFP_KERNEL);
	if (!f1a) {
	...
	f1a->button_data_buffer = devm_kzalloc(&fc->dev,
	...
	f1a->button_map = devm_kzalloc(&fc->dev,
	...
	if (f1a->query.has_general_control) {
		f1a->control.reg_0 = devm_kzalloc(&fc->dev,
	...
	if (f1a->query.has_tx_rx_map) {
		f1a->control.reg_3_4 = devm_kzalloc(&fc->dev,
		...
		f1a->control.reg_3_4->regs = devm_kzalloc(&fc->dev,
	...
	if (f1a->query.has_release_threshold) {
		f1a->control.reg_6 = devm_kzalloc(&fc->dev,
	...
	if (f1a->query.has_strongestbtn_hysteresis) {
		f1a->control.reg_7 = devm_kzalloc(&fc->dev,
	...
	if (f1a->query.has_filter_strength) {
		f1a->control.reg_8 = devm_kzalloc(&fc->dev,
...
static int rmi_f1a_register_device(struct rmi_function_container *fc)
{
	...
	sprintf(f1a->input_phys, "%s/input0", dev_name(&fc->dev));
...
static int f1a_device_init(struct rmi_function_container *fc)
{
	int rc;
...
static int f1a_remove_device(struct device *dev)
{
	struct f1a_data *f1a;
...
static int rmi_f1a_attention(struct rmi_function_container *fc,
...
static __devinit int f1a_probe(struct device *dev)
{
