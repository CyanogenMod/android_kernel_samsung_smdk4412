...
{
	int reg_val = 32;
	struct rmi_function_container *fc = container_of(gc,
	...
	rc = rmi_write_block(rmi_dev, f30->data.datareg_0->address,
				(u8 *)reg_val, bytecnt);
	if (rc < 0) {
		dev_err(&fc->dev, "%s error %d: Could not read control 0 to 0x%x\n",
...
static inline int rmi_f30_alloc_memory(struct rmi_function_container *fc)
{
	...
	f30 = devm_kzalloc(&fc->dev, sizeof(struct rmi_fn_30_data), GFP_KERNEL);
	if (!f30) {
	...
	f30->button_data_buffer =
	    devm_kzalloc(&fc->dev, f30->button_bitmask_size, GFP_KERNEL);
	if (!f30->button_data_buffer) {
		dev_err(&fc->dev, "Failed to allocate button data buffer.\n");
		return -ENOMEM;
	}

	f30->gpioled_map = devm_kzalloc(&fc->dev,
...
int rmi_f30_attention(struct rmi_function_container *fc,
...
static int rmi_f30_register_device(struct rmi_function_container *fc)
{
	...
	if (driver->set_input_params) {
		...
		if (rc < 0) {
			dev_err(&fc->dev, "%s: Error in setting input device.\n",
			__func__);
			goto error_free_device;
		}
	}
	sprintf(f30->input_phys, "%s/input0", dev_name(&fc->dev));
...
static inline int rmi_f30_initialize(struct rmi_function_container *fc)
{
	...
	control->reg_0->regs = devm_kzalloc(&fc->dev, regs_size, GFP_KERNEL);
	...
	/* reg_2 */
	instance_data->control.reg_2 = devm_kzalloc(&fc->dev,
	...
	regs_size = max(sizeof(struct f30_gpio_ctrl_2n)*reg_flg*
					instance_data->gpioled_bitmask_size, 1);
	control->reg_2->regs = devm_kzalloc(&fc->dev, regs_size, GFP_KERNEL);
	...
	/* reg_3 */
	instance_data->control.reg_3 = devm_kzalloc(&fc->dev,
	...
	regs_size = max(sizeof(struct f30_gpio_ctrl_3n) * reg_flg *
					instance_data->gpioled_bitmask_size, 1);
	control->reg_3->regs = devm_kzalloc(&fc->dev, regs_size, GFP_KERNEL);
	...
	/* reg_4 */
	control->reg_4 = devm_kzalloc(&fc->dev,
	...
	control->reg_4->regs = devm_kzalloc(&fc->dev, regs_size, GFP_KERNEL);
	...
	/* reg_5 */
	control->reg_5 = devm_kzalloc(&fc->dev,
	...
	control->reg_5->regs = devm_kzalloc(&fc->dev, regs_size, GFP_KERNEL);
	...
	/* reg_6 */
	control->reg_6 = devm_kzalloc(&fc->dev,
	...
	control->reg_6->regs = devm_kzalloc(&fc->dev, regs_size, GFP_KERNEL);
	...
	f30_attrs_regs_exist[reg_num] = true;
	control->reg_7->regs = devm_kzalloc(&fc->dev, regs_size, GFP_KERNEL);
	...
	/* reg_8 */
	control->reg_8 = devm_kzalloc(&fc->dev,
	...
	f30_attrs_regs_exist[reg_num] = true;
	control->reg_8->regs =
			devm_kzalloc(&fc->dev, regs_size, GFP_KERNEL);
	...
	/* reg_9 */
	control->reg_9 = devm_kzalloc(&fc->dev,
	...
	/* data reg_0 */
	instance_data->data.datareg_0 = devm_kzalloc(&fc->dev,
...
static int f30_device_init(struct rmi_function_container *fc)
{
	int rc;
	struct rmi_fn_30_data *f30 = fc->data;
...
static int f30_remove_device(struct device *dev)
{
	struct rmi_function_container *fc = to_rmi_function_container(dev);

	dev_dbg(&fc->dev, "Removing F30.");

	rmi_f30_free_memory(fc);
	return 0;
}

static __devinit int f30_probe(struct device *dev)
{
