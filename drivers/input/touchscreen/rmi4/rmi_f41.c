...
static int rmi_f41_initialize(struct rmi_function_container *fc)
{
	...
	f41 = devm_kzalloc(&fc->dev, sizeof(struct rmi_fn_41_data),
				GFP_KERNEL);
	if (!f41) {
	...
	if (f41->query.has_reduced_reporting == 1 ||
		...
		f41->control.reg_0 = devm_kzalloc(&fc->dev,
	...
	if (f41->query.number_of_buttons > 0) {
		f41->control.reg_1 = devm_kzalloc(&fc->dev,
	...
	f41->control.reg_2__5 =	devm_kzalloc(&fc->dev,
	...
	if (f41->query.has_reduced_reporting == 1) {
		f41->control.reg_6__9 = devm_kzalloc(&fc->dev,
	...
	if (f41->query.has_single_tap == 1) {
		f41->control.reg_10 = devm_kzalloc(&fc->dev,
	...
	f41->data_block = devm_kzalloc(&fc->dev, sizeof(union f41_ap_data_0__3)
	...
	f41->button_map = devm_kzalloc(&fc->dev,
...
static int rmi_f41_config(struct rmi_function_container *fc)
{
	...
	if (f41->attrs_ctrl_regs_exist[f41_ctrl_reg_1]) {
		retval = rmi_write_block(fc->rmi_dev,
					...
					sizeof(f41->control.reg_1->regs));
		if (retval < 0) {
			dev_err(&fc->dev, "%s : Could not write reg1 to 0x%x\n",
	...
	if (f41->attrs_ctrl_regs_exist[f41_ctrl_reg_2__5]) {
		retval = rmi_write_block(fc->rmi_dev,
					...
					sizeof(f41->control.reg_2__5->regs));
		if (retval < 0) {
			dev_err(&fc->dev, "%s : Could not write reg2_5 to 0x%x\n",
					__func__,
					f41->control.reg_2__5->address);
			return retval;
		}
	...
	if (f41->attrs_ctrl_regs_exist[f41_ctrl_reg_6__9]) {
		retval = rmi_write_block(fc->rmi_dev,
					...
					sizeof(f41->control.reg_6__9->regs));
		if (retval < 0) {
			dev_err(&fc->dev, "%s : Could not write reg6_9 to 0x%x\n",
					__func__,
					f41->control.reg_6__9->address);
			return retval;
		}
	...
	if (f41->attrs_ctrl_regs_exist[f41_ctrl_reg_10]) {
		retval = rmi_write_block(fc->rmi_dev,
					...
					sizeof(f41->control.reg_10->regs));
		if (retval < 0) {
			dev_err(&fc->dev, "%s : Could not write reg10 to 0x%x\n",
...
static int f41_remove_device(struct device *dev)
{
	struct rmi_fn_41_data *f41;
...
static int rmi_f41_register_device(struct rmi_function_container *fc)
{
	...
	sprintf(f41->input_phys, "%s/input0", dev_name(&fc->dev));
...
static int rmi_f41_attention(struct rmi_function_container *fc,
...
static int f41_device_init(struct rmi_function_container *fc)
{
	int retval = 0;
...
static __devinit int f41_probe(struct device *dev)
{
