...
static int rmi_f21_alloc_memory(struct rmi_function_container *fc)
{
	...
	f21 = devm_kzalloc(&fc->dev, sizeof(struct rmi_fn_21_data), GFP_KERNEL);
	if (!f21) {
...
static int rmi_f21_initialize(struct rmi_function_container *fc)
{
	...
	f21->control.reg_0__3 = devm_kzalloc(&fc->dev,
	...
	f21->control.reg_4 = devm_kzalloc(&fc->dev,
	...
	f21->control.reg_4->regs = devm_kzalloc(&fc->dev,
	...
	f21->control.reg_5 = devm_kzalloc(&fc->dev,
	...
	f21->control.reg_5->regs = devm_kzalloc(&fc->dev,
	...
	f21->control.reg_6 = devm_kzalloc(&fc->dev,
	...
	f21->control.reg_6->regs = devm_kzalloc(&fc->dev,
	...
	f21->control.reg_7 = devm_kzalloc(&fc->dev,
	...
	f21->control.reg_7->regs = devm_kzalloc(&fc->dev,
	...
	f21->control.reg_8 = devm_kzalloc(&fc->dev,
	...
	f21->control.reg_8->regs = devm_kzalloc(&fc->dev,
	...
	f21->control.reg_9 = devm_kzalloc(&fc->dev,
	...
	f21->control.reg_9->regs =
			devm_kzalloc(&fc->dev, sizeof(struct f21_2df_control_9n)
	...
	f21->data.reg_0__1.force_hi_lo = devm_kzalloc(&fc->dev,
	...
	f21->data.reg_2 = devm_kzalloc(&fc->dev,
...
static int rmi_f21_config(struct rmi_function_container *fc)
{
	...
	retval = rmi_write_block(fc->rmi_dev, data->control.reg_0__3->address,
				(u8 *)data->control.reg_0__3,
				sizeof(data->control.reg_0__3->regs));
	if (retval < 0) {
		dev_err(&fc->dev, "%s : Could not write reg_0_3 to 0x%x\n",
			__func__, data->control.reg_0__3->address);
		return retval;
	}

	retval = rmi_write_block(fc->rmi_dev, data->control.reg_4->address,
			(u8 *) data->control.reg_4->regs,
			data->control.reg_4->length);
	if (retval < 0) {
		dev_err(&fc->dev, "%s : Could not write reg_4 to 0x%x\n",
			__func__, data->control.reg_4->address);
		return retval;
	}

	retval = rmi_write_block(fc->rmi_dev, data->control.reg_5->address,
			(u8 *) data->control.reg_5->regs,
			data->control.reg_5->length);
	if (retval < 0) {
		dev_err(&fc->dev, "%s : Could not write reg_5 to 0x%x\n",
			__func__, data->control.reg_5->address);
		return retval;
	}

	retval = rmi_write_block(fc->rmi_dev, data->control.reg_6->address,
			(u8 *) data->control.reg_6->regs,
			data->control.reg_6->length);
	if (retval < 0) {
		dev_err(&fc->dev, "%s : Could not write reg_6 to 0x%x\n",
			__func__, data->control.reg_6->address);
		return retval;
	}

	retval = rmi_write_block(fc->rmi_dev, data->control.reg_7->address,
			(u8 *) data->control.reg_7->regs,
			data->control.reg_7->length);
	if (retval < 0) {
		dev_err(&fc->dev, "%s : Could not write reg_7 to 0x%x\n",
			__func__, data->control.reg_7->address);
		return retval;
	}

	retval = rmi_write_block(fc->rmi_dev, data->control.reg_8->address,
			(u8 *) data->control.reg_8->regs,
			data->control.reg_8->length);
	if (retval < 0) {
		dev_err(&fc->dev, "%s : Could not write reg_8 to 0x%x\n",
			__func__, data->control.reg_8->address);
		return retval;
	}

	retval = rmi_write_block(fc->rmi_dev, data->control.reg_9->address,
			(u8 *) data->control.reg_9->regs,
			data->control.reg_9->length);
	if (retval < 0) {
		dev_err(&fc->dev, "%s : Could not write reg_9 to 0x%x\n",
...
static int f21_device_init(struct rmi_function_container *fc)
{
	int retval = 0;
...
static int f21_remove_device(struct device *dev)
{
	struct rmi_function_container *fc = to_rmi_function_container(dev);

	dev_dbg(&fc->dev, "Removing F21.");
...
static __devinit int f21_probe(struct device *dev)
{
	struct rmi_function_container *fc;
