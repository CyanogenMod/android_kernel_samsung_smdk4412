...
static inline unsigned short mdnie_effect_master_hook(struct mdnie_info *mdnie, unsigned short val) {
	...
	dev_info(mdnie->dev, "(0/2) hooking effect master reg, now %u\n", val);
	// always try and turn on rgb
	// TODO: only do it if we're actually going to modify the values (i.e. *_adj < 255)
	dev_info(mdnie->dev, "(1/2) hooking effect master reg, now %u\n", val);
	val |= MDNIE_SCR_MASK;
	dev_info(mdnie->dev, "(2/2) hooking effect master reg, now %u\n", val);
